/**
  ...
  *       Last update:   23/01/2025 19:50 GMT+0200
  ******************************************************************************
...
limbSftyFwConf_t mnCtrlTskConf{
   ...
   .lsSwExecTskPrrtyCnfg = 1, //todo Changing to a lower priority (higher number) fails in WOKWI!!
};
...
/*  //Create the task to keep the lftHndSwtch outputs updated to reflect the MPB's state
   xReturned = xTaskCreatePinnedToCore(
      lftHndSwtchOtptsUpdTsk,  // Callback function/task to be called
      "LftHndSwOtpTsk",  // Name of the task
      1536,   // Stack size (in bytes in ESP32, words in FreeRTOS), the minimum value is in the config file, for this is 768 bytes
      &lftHndHwOtpts,  // Pointer to the parameters for the function to work with
      _exePrty,  // Priority level given to the task
      &lftHndHwOtpsChng, // Task handle
      appCpuCore // Run in the App Core if it's a dual core mcu (ESP-FreeRTOS specific)
   );
   if(xReturned != pdPASS)
      Error_Handler();

  //Create the task to keep the rgttHndSwtch outputs updated to reflect the MPB's state
   xReturned = xTaskCreatePinnedToCore(
      rghtHndSwtchOtptsUpdTsk,  // Callback function/task to be called
      "RghtHndSwOtpTsk",  // Name of the task
      1536,   // Stack size (in bytes in ESP32, words in FreeRTOS), the minimum value is in the config file, for this is 768 bytes
      &rghtHndHwOtpts,  //  Pointer to the parameters for the function to work with
      _exePrty,  //  Priority level given to the task
      &rghtHndHwOtpsChng, //  Task handle
      appCpuCore //  Run in the App Core if it's a dual core mcu (ESP-FreeRTOS specific)
   );
   if(xReturned != pdPASS)
      Error_Handler();

  //Create the task to keep the ftSwtch outputs updated to reflect the MPB's state
   xReturned = xTaskCreatePinnedToCore(
      ftSwtchOtptsUpdTsk,  // Callback function/task to be called
      "FtSwOtpTsk",  // Name of the task
      1536,   //Stack size (in bytes in ESP32, words in FreeRTOS), the minimum value is in the config file, for this is 768 bytes
      &ftSwtchIsEnbldOtpt,  // Pointer to the parameters for the function to work with
      _exePrty,  // Priority level given to the task: use the same as the Software Timers priority level      
      &ftHwOtpsChng, // Task handle
      appCpuCore // Run in the App Core if it's a dual core mcu (ESP-FreeRTOS specific)
   );
   if(xReturned != pdPASS)
      Error_Handler();   
   
   // Configure the parameters for the underlying switches
   stampSftySwtch.getLftHndSwtchPtr()->setStrtDelay(150);
   stampSftySwtch.getLftHndSwtchPtr()->setTaskToNotify(lftHndHwOtpsChng);
   stampSftySwtch.getLftHndSwtchPtr()->setVoidTime(6000);

   stampSftySwtch.getRghtHndSwtchPtr()->setStrtDelay(150);
   stampSftySwtch.getRghtHndSwtchPtr()->setTaskToNotify(rghtHndHwOtpsChng);
   stampSftySwtch.getRghtHndSwtchPtr()->setVoidTime(6000);

   stampSftySwtch.getFtSwtchPtr()->setStrtDelay(100);
   stampSftySwtch.getFtSwtchPtr()->setTaskToNotify(ftHwOtpsChng);
*/
   ...
   for(;;){
      // FTPO Trying to save resources by not executing this loop constantly, but only at the same pace as the switch update timer
      *loopTimerStartTime = xTaskGetTickCount() / portTICK_RATE_MS;
      ...
      xTaskDelayUntil(loopTimerStartTime, 50);
